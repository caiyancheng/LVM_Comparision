...
json_data_path = rf'new_data_logs/dinov2/different_luminance/dinov2_test_on_gabors_different_C_b_contour_plot_ppd_{ppd}_final.json'
with open(json_data_path, 'r') as fp:
    plot_json_data = json.load(fp)

backbone_name_list = plot_json_data['backbone_name']
plot_C_b_matrix_list = plot_json_data['C_b_matrix']
...
for backbone_index in range(len(backbone_name_list)):
    ...
    for ax in axs_all:
        if index < 6:
            cs = ax.contour(plot_L_b_matrix, plot_contrast_matrix, plot_similarity_list[index], levels=20)
            ax.set_title(title_name_list[index])
            ax.set_yticks(y_contrast_ticks)
            ax.set_ylabel('Contrast')
        else:
            cs = ax.contour(plot_L_b_matrix, 1 / plot_contrast_matrix, plot_similarity_list[index-6], levels=20)
