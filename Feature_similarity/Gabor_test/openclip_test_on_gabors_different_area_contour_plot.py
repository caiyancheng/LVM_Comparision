...
if not test_continue:
    ...
    df = pd.read_csv(os.path.join(save_root_path, f'openclip_test_on_gabors_different_area_contour_plot_ppd_{default_ppd}_temporary.csv'))
        ...
        for contrast_index in range(len(contrast_list)):
            ...
            if test_continue:
                ...
                if len(check_df) == 1:
                    ...
                    plot_final_feature_L1_similarity_matrix[R_index, contrast_index] = check_df['final_feature_L1_similarity']
                    plot_final_feature_L2_similarity_matrix[R_index, contrast_index] = check_df['final_feature_L2_similarity']
                    plot_final_feature_cos_similarity_matrix[R_index, contrast_index] = check_df['final_feature_cos_similarity']
                    continue
                elif len(check_df) > 1:
            ...
            T_vid, R_vid = generate_gabor_patch(W=default_W, H=default_H, R=R_value, rho=default_rho, O=default_O,
                                                ...
                                                color_direction='ach')
            T_vid_c = Image.fromarray((display_encode_tool.L2C(T_vid) * 255).astype(np.uint8))
            R_vid_c = Image.fromarray((display_encode_tool.L2C(R_vid) * 255).astype(np.uint8))
...
df.to_csv(
    ...
    index=False)
with open(
        os.path.join(save_root_path, f'openclip_test_on_gabors_different_area_contour_plot_ppd_{default_ppd}_final.json'),
        'w') as fp:
